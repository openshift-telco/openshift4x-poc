...
usage() {
    ...
    echo -e "\t\t(extras) [ get_images | prep_installer | prep_images | setup_local_registry ]"
}
setup_local_registry() {
    yum install -y docker-distribution skopeo podman
    echo "Please note if you are using RHEL7 subscriptions following packages needs to be manually pre-installed in order to have podman and httpd-tools"
    echo "libnet-1.1.6-7.el7.x86_64.rpm python-IPy-0.75-6.el7.noarch.rpm apr-util-1.5.2-6.el7.x86_64.rpm"
    echo "Generating the required certificate file for the docker-distribution service."
    mkdir /etc/docker-distribution/certs ;cd /etc/docker-distribution/certs
    openssl req -newkey rsa:4096 -nodes -sha256 -keyout domain.key -x509 -days 365 -out domain.crt
    echo "Generating htpasswd based authentication"
    ...
    echo "Replace the "host" line appropriately with the ${BASTIAN_FQDN}"
    echo "Replace the "secret" with a random value"
    echo "Replace the rootdirectory as required"
    cp ./utils/registry_config.yaml /etc/docker-distribution/registry/config.yml
    systemctl start docker-distribution
    firewall-cmd --add-port=5000/tcp --zone=internal --permanent
    firewall-cmd --add-port=5000/tcp --zone=public --permanent
    firewall-cmd --reload
    echo "Docker client configuration"
    mkdir /etc/docker/certs.d/${BASTIAN_FQDN}:5000    
    echo "Trust this certificate"
    cp /etc/docker-distribution/certs/domain.crt /etc/docker/certs.d/registry-internal\:5000/domain.crt 
    cp /etc/docker-distribution/certs/domain.crt    /etc/pki/ca-trust/source/anchors/${BASTIAN_FQDN}.crt
    update-ca-trust 
    echo "Add the newly created registry to the /etc/containers/registries.conf"
    cp ./utils/local_registries.conf /etc/containers/registries.conf
    echo "Restart the docker-distribution service"
